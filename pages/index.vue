<template>
  ...
  >
    <div
      class="w-full h-full bg-white/30 backdrop-blur-[218px] grid place-items-center relative"
    >
      <div
        class="w-80 min-h-[20rem] h-auto bg-white/10 backdrop-blur-[100px] rounded-lg z-10 relative border border-white/50"
      >
        <div
        ...
        <form @submit.prevent="login()" class="flex flex-col gap-2 p-5">
          ...
          <button
            v-if="isLoading"
            class="bg-gray-600 text-white py-2 rounded-lg mt-6"
          >
            <IconsCircelLoder />
          </button>
          <button
            v-else
            class="py-2 rounded-lg mt-6 font-semibold"
